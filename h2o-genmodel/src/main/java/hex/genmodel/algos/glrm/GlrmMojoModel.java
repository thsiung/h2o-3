...
public class GlrmMojoModel extends MojoModel {
  ...
  public boolean _predictFromModel = false;  // change to true if used from model, added to ensure reproducibility
  ...
  protected final double lmulNumCol(double[] x, int j) {
    ...
    if (_transposed) {
      for (int k = 0; k < rank(); k++)
        prod += x[k] * _archetypes_raw[cidx][k];
    } else {
      for (int k = 0; k < rank(); k++)
        prod += x[k] * _archetypes_raw[k][cidx];
    }
    return prod;
  }
  ...
  protected final double[] lmulCatBlock(double[] x, int j) {
    ...
    if (_transposed) {
      for (int level = 0; level < catColJLevel; level++) {
        ...
        for (int k = 0; k < rank(); k++)
          prod[level] += x[k] * _archetypes_raw[cidx][k];
      }
    } else {
      for (int level = 0; level < catColJLevel; level++) {
        int cidx = getCatCidx(j,level);
        for (int k = 0; k < rank(); k++)
          prod[level] += x[k] * _archetypes_raw[k][cidx];
      }
    }
    return prod;
  }
  ...
  public int rank() {
    return _transposed ? _archetypes_raw[0].length : _archetypes_raw.length;
  }
